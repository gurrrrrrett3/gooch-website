...
import mmInterface from "./mmInterface";
import mmApi from "./mmFetch";
import { mmFetchPlayersReturn } from "./mmTypes";
export default class mmCaching {
  ...
  private saveCache(data: any, file: string) {
    fs.writeFileSync(path.resolve(`./data/mm/cache/${file}.json`), JSON.stringify(data));
  }
  ...
  public saveCachedPlayer(username: string, data: any) {
    fs.writeFileSync(path.resolve(`./data/mm/cache/players/${username}.json`), JSON.stringify(data));
  }

  public loadCachedPlayer(username: string) {
    return JSON.parse(fs.readFileSync(path.resolve(`./data/mm/cache/players/${username}.json`), "utf8"));
  }

  public getCacheData() {
    return JSON.parse(fs.readFileSync(path.resolve(`./data/mm/cache/data.json`), "utf8"));
  }

  private saveCacheData(data: any) {
    fs.writeFileSync(path.resolve(`./data/mm/cache/data.json`), JSON.stringify(data));
  }

  private checkForCacheData() {
    if (!fs.existsSync(path.resolve(`./data/mm/cache/data.json`))) {
      this.saveCacheData({
        timestamp: Date.now(),
        report: {
            players: 0,
  ...
  private checkForCacheFiles() {
    if (!fs.existsSync(path.resolve(`./data/mm/cache/players`))) {
      fs.mkdirSync(path.resolve(`./data/mm/cache/players`), { recursive: true });
    }
  }
}
